  ...
  std::pair<double, double> wait() {
    ...
    for (size_t i = 0; i < num_clients_; ++i) {
      for(size_t j = 0; j < num_ops_; ++j) {
        try {
          d_clients_[i]->remove(std::string("/") + std::to_string(i) + std::string("/") +  std::to_string(j));
 ...
 public:
  ...
  void run() override {
    for (size_t i = 0; i < num_clients_; ++i) {
      workers_[i] = std::thread([i, this]() {
        for (size_t j = 0; j < num_ops_; ++j) {
          d_clients_[i]->open_or_create(std::string("/") + std::to_string(i) + std::string("/") +  std::to_string(j), "hashtable", std::string("local") + std::to_string(j) + std::string("-") + std::to_string(i));
        }
        auto bench_begin = time_utils::now_us();
        uint64_t tot_time = 0, t0, t1 = bench_begin;
        size_t j;
        for (j = 0; j < num_ops_; ++j) {
          t0 = time_utils::now_us();
          d_clients_[i]->add_block(std::string("/") + std::to_string(i) + std::string("/") +  std::to_string(j), std::to_string(j) + std::string("-") +  std::to_string(i), "regular");
          t1 = time_utils::now_us();
          tot_time += (t1 - t0);
        }
        latency_[i] = (double) tot_time / (double) j;
...
class renew_leases_benchmark : public directory_benchmark {
 public:
  ...
  void run() override {
    for (size_t i = 0; i < num_clients_; ++i) {
        for (size_t j = 0; j < num_ops_; ++j) {
          d_clients_[0]->open_or_create(std::string("/") + std::to_string(i) + std::string("/") +  std::to_string(j), "hashtable", std::string("local") + std::to_string(j) + std::string("-") + std::to_string(i));
        }
    }
    for (size_t i = 0; i < num_clients_; ++i) {
      workers_[i] = std::thread([i, this]() {
        ...
        for (j = 0; j < num_ops_; ++j) {
          ...
          paths.push_back(std::string("/") + std::to_string(i) + std::string("/") + std::to_string(j));
          t0 = time_utils::now_us();
...
int main() {
 ...
 //FIXME generate path here
  ...
  for (const auto &op_type:op_type_set) {
    for (int i = 1; i <= 64; i *= 2) { // FIXME fix this loop number
      int num_clients = i;

...
namespace confluo {
namespace storage {
typedef jiffy::storage::binary msg_type;
typedef jiffy::storage::block_memory_allocator<msg_type> file_allocator_type;
class allocator {
 public:
  static file_allocator_type &instance() {
    static file_allocator_type alloc;
    return alloc;
  }
};
  ...
  void decrement_compare_dealloc() {
    ...
    if (internal_ptr != nullptr && ref_counts_ != nullptr) {
      ...
      if (uses_first_count && ref_counts_->decrement_first_and_compare()) {
        ...
        allocator::instance().deallocate(internal_ptr);
